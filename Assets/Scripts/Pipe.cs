...
public class Pipe : MonoBehaviour {
	...
	private void CreateRing(float u, int i)
	{
		float vStep = (2f * Mathf.PI) / pipeSegmentCount;
		int vertexPerRing = pipeSegmentCount * 4;
		int ringOffset = vertexPerRing;

		Vector3 v1 = i < vertexPerRing ? GetPointOnTorus(0f, 0f) : vertices[i - ringOffset + 2];
		Vector3 v2 = GetPointOnTorus(u, 0f);

		for (int v = 1; v <= pipeSegmentCount; v++, i += 4)
		{
			ringOffset = i < vertexPerRing ? 0 : vertexPerRing;
			vertices[i] = v1 = i < vertexPerRing ? v1 : vertices[i - ringOffset + 2];
			vertices[i + 1] = v1 = i < vertexPerRing ? GetPointOnTorus(0f, v * vStep) : vertices[i - ringOffset + 3];
			vertices[i + 2] = v2;
			vertices[i + 3] = v2 = GetPointOnTorus(u, v * vStep);
		}
	}
	private void SetUV () {
